...
def downloadGoogleDrive(file_ID, singleID, taxExempt="False", patronName='', projectNumber=''):
    ...
    file.close()
    if passFail == "GDrive Error":
        commentStatus(singleID, "We are currently experiencing an issue with google drive links and our automation system. A human will manually review this ticket as soon as possible!")
        changeStatus(singleID, "11")
        if os.path.exists("drivedownloads/" + singleID + ".gcode"):
            os.remove("drivedownloads/" + singleID + ".gcode")
    elif passFail == "Bad G-code":
    ...
    elif passFail == "Bad Filetype":
        commentStatus(singleID, "It appears you may have submitted an invalid filetype! We can only accept .gcode files. Please reach out if you have questions or see our getting started video: https://youtu.be/kGpXsIX9E_k")
        changeStatus(singleID, "11")
        changeStatus(singleID, "21")
        changeStatus(singleID, "131")
        if os.path.exists("drivedownloads/" + singleID + ".gcode"):
...
def customGcodeCheck(file, ticketID='', taxExempt=False, projectNumber='', patronName=''):
    ...
    try:
        ...
        try:
            ...
            return "Bad Filetype", file
    if "<html lang=en>" in splitFile[0]:
        return "GDrive Error", file
    ...
    for line in splitFile:
        ...
        if "M0 " in line:
            if "Press To Start" in line and m0startLine == 0:
                """
                Ben is messing with stuff here
